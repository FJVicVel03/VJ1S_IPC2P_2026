...
namespace Ejemplo_9.Models
{
    ...
    public class RedSatelitalPlano
    {
        ...
        public void Insert(int row, int col, string id, string nombre, string ip)
        {
            if (row < 0 || col < 0)
            {
                throw new ArgumentException("Las coordenadas de fila y columna deben ser no negativas.");
            }
        ...
        public string GenerarCodigoDot()
        {
            ...
            while (rowNode != null)
            {
                ...
                while (node != null)
                {
                    ...
                    dot.AppendLine($"        <TABLE BORDER=\"0\" CELLBORDER=\"1\" CELLSPACING=\"0\" BGCOLOR=\"#D5F5E3\">");
            ...
            while (rowNode != null)
            {
                if (rowNode.Access != null)
                {
                    ...
                    while (node != null)
                    {
                        ...
                        if (node.Right != null)
                        {
                            dot.AppendLine($"    node_{node.Row}_{node.Col}:right -> node_{node.Right.Row}_{node.Right.Col}:left [dir=both, color=blue];");
                        }
                        node = node.Right;
                    }
                }
            ...
            while (colNode != null)
            {
                if (colNode.Access != null)
                {
                    ...
                    while (node != null)
                    {
                        ...
                        if (node.Down != null)
                        {
                            dot.AppendLine($"    node_{node.Row}_{node.Col}:down -> node_{node.Down.Row}_{node.Down.Col}:up [dir=both, color=red];");
                        }
                        node = node.Down;
                    }
                }
